...
while not glfw.window_should_close(handle):
    ...
    imgui.begin("Ledger", None, flags=imgui_flags);
    
    if imgui.begin_main_menu_bar():
        if imgui.begin_menu("File"):
            if imgui.begin_menu("Open"):
                receipt_dir = "./receipts";
                for path in os.listdir(receipt_dir):
                    if path.endswith(".csv"):
                        if imgui.menu_item(path)[0]:
                            ledger_path = os.path.join(receipt_dir, path);
                            ledger = Ledger();
                            with open(ledger_path) as file:
                                reader = csv.reader(file);
                                for row in reader:
                                    purchase = Purchase(row[0], float(row[1]), int(row[2]), False);
    ...
    if ledger == None:
        ...
        if imgui.begin_tab_bar("Tabs"):
            ...
            if imgui.begin_tab_item("Tables")[0]:
                if imgui.collapsing_header("Receipt")[0]:
                    if imgui.begin_table("Receipt Table", 3, imgui.TABLE_BORDERS | imgui.TABLE_SIZING_FIXED_SAME):
                ...
                imgui.separator();
                for actor in default_actors:
                    if imgui.collapsing_header(actor)[0]:
                        if imgui.begin_table(f"{actor} Table", 4, imgui.TABLE_BORDERS | imgui.TABLE_SIZING_FIXED_SAME):
                            imgui.table_setup_column("Item");
                            imgui.table_setup_column("Cost");
                            imgui.table_setup_column("Quantity");
